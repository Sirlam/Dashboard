...
namespace Dashboard.Utils
{
    public class FcyUtil
    {
        ...
        public List<FcyStateSummary> GetFcyTodayStateSummary(string fcyType)
        {
            List<FcyStateSummary> fcyStateSummaries = new List<FcyStateSummary>();
            ...
            using (SqlConnection con = new SqlConnection(_sqlCs))
            {
                try
                {
                    ...
                    while (rdr.Read())
                    {
                        var transactionSummary = new FcyStateSummary();
                        ...
                        fcyStateSummaries.Add(transactionSummary);
                    }
                }
                catch (Exception e)
                {
            ...
            return fcyStateSummaries;
        }

        private DateTime GetLastFetchDateTime()
        {
            const string sqlSelect = "SELECT TOP 1 spooled_time FROM fcy_cards ORDER BY ID DESC ";
        ...
        public IEnumerable<FcyDetail> GetFcyDetails()
        {
            string sqlSelect =
            ...
            List<FcyDetail> fcyDetails = new List<FcyDetail>();
            ...
            using (SqlConnection con = new SqlConnection(_fepCs))
            {
                try
                {
                    ...
                    while (rdr.Read())
                    {
                        FcyDetail fcyDetail = new FcyDetail();
                        fcyDetail.TransactionType = rdr["tran_type"].ToString();
                        fcyDetail.ChannelType = rdr["CHANNEL_TYPE"].ToString();
                        fcyDetail.TransactionCount = Convert.ToInt32(rdr["tran_count"].ToString());
                        fcyDetail.TransactionVolume = Convert.ToDecimal(rdr["tot_vol"].ToString());
                        object j = rdr["rsp_code"].GetType().Name;
                        if (!rdr["rsp_code"].GetType().Name.Equals("DBNull"))
                            fcyDetail.TransactionResponseCode = rdr["rsp_code"].ToString();
                        else
                        {
                            fcyDetail.TransactionResponseCode = null;
                        }
                        fcyDetail.SpooledTime = DateTime.Now;
                        fcyDetails.Add(fcyDetail);
                }
                }
                catch (Exception e)
                {
        ...
        public int StageFcyData()
        {
            IEnumerable<FcyDetail> fcyDetails = GetFcyDetails();
            ...
            using (SqlConnection con = new SqlConnection(_sqlCs))
            {
                ...
                {
                    con.Open();
                    foreach (var fcyDetail in fcyDetails)
                    {
                        const string insQuery =
                            "BEGIN Insert Into fcy_cards (tran_type,tran_count,rsp_code,channel_type,spooled_time,tot_vol)" +
                            "  values (@tran_type,@tran_count,@rsp_code,@channel_type,@spooledtime,@tot_vol); END;";
                        SqlCommand cmd = new SqlCommand(insQuery, con);

                        if (fcyDetail.TransactionResponseCode != null)
                        {
                            cmd.Parameters.AddWithValue("@rsp_code", fcyDetail.TransactionResponseCode);
                        }
                        else
                        {
                        ...
                        cmd.Parameters.AddWithValue("@tran_count", SqlDbType.Int).Value = fcyDetail.TransactionCount;
                        cmd.Parameters.AddWithValue("@tot_vol", SqlDbType.Decimal).Value = fcyDetail.TransactionVolume;
                        cmd.Parameters.AddWithValue("@tran_type", SqlDbType.NVarChar).Value = fcyDetail.TransactionType;
                        cmd.Parameters.AddWithValue("@channel_type", SqlDbType.NVarChar).Value = fcyDetail.ChannelType;
                        cmd.Parameters.AddWithValue("@spooledtime", SqlDbType.DateTime).Value = fcyDetail.SpooledTime;
