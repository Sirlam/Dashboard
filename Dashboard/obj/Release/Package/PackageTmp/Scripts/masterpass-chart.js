    ...
    function weeklySalesChartData() {
        ...
        $.ajax({
            ...
            success: function (data) {
                //console.log(data);
                for (var i = 0; i < data.length; i++) {
                    count[i] = data[i]['TotalTransaction'];
                    successCount[i] = data[i]['SuccessCount'];
                    failedCount[i] = data[i]['FailedCount'];
                    dateTime[i] = data[i]['SalesDate'];
                    //console.log('rib ' + rib);
                }
            }
        });
        ...
        var ctx = document.getElementById("weekly_sales_metrics").getContext("2d");
        window.myBar = new Chart(ctx, {
            type: 'bar',
            data: trendingChartData,
            options: {
